 ...
 * IDT setup, PIC remapping, exception and IRQ dispatch
 */
#include "idt.h"
#include "io.h"
...
/*
 * remap the PIC so IRQs 0-15 land at vectors 32-47
 * instead of 0-15 where they'd clash with CPU exceptions
 */
...
void irq_handler(registers_t *regs)
{
    ...
    /* send EOI — PIC2 first if it was a cascade IRQ */
    if (irq >= 8)
...
void idt_init(void)
{
    ...
    /* remap PIC then install IRQ handlers at vectors 32-47 */
    pic_remap();
    ...
    /* syscall vector — ring 3 callable (DPL=3) */
