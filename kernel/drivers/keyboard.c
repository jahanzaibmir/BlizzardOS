...
/*
 * send the current LED state to the PS/2 controller.
 * uses a timeout on every busy-wait so we never deadlock.
 * safe to call from the IRQ handler, but NOT before sti() —
 * see the comment in keyboard_init() for why.
 */
...
void keyboard_init(void)
{
     ...
     *
     * keyboard_init() runs before sti() in kernel_main. the PS/2 controller
     * sends an ACK after the 0xED command, but with interrupts disabled we
     * never process it — the input-buffer-full bit never clears and the second
     * busy-wait spins forever, hanging the boot.
     *
     * LEDs will sync up automatically on the first CapsLock/NumLock/ScrollLock
     * keypress.
     */
}
