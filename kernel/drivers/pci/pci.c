 ...
 *
 * kprintf routes through the VGA terminal which can crash before the
 * framebuffer is ready. serial_write is always safe after serial_init().
 */
#include "pci.h"
...
/* ── build PCI config space address ─────────────────────────────────────── */
...
int pci_find_device(uint16_t vendor, uint16_t device, pci_device_t *dev)
{
    for (uint8_t slot = 0; slot < 32; slot++) {
        ...
        if (vid == 0xFFFF) continue;  /* empty slot */
...
/* ── scan bus 0 and print everything found via serial ───────────────────── */
